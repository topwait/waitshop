	...
	<view class="container">
		...
		<mescroll-body ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" :up="upOption">
			<view class="index-record-widget u-margin-tb-20 u-bg-white">
				<view class="record-item u-padding-30 u-border-bottom" v-for="(item, index) in recordList" :key="index">
					<view class="u-flex u-row-between">
						<view class="u-font-30 u-margin-bottom-10">{{item.tips}}</view>
						<view class="u-font-34 u-color-major">{{item.arrival}}</view>
					</view>
					<view class="u-font-24 u-color-muted">{{item.pay_time}}</view>
				</view>
			</view>
		</mescroll-body>
