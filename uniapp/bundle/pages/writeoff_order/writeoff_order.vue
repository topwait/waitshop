			...
			<view class="index-order-widget">
				<view class="pack-order-item"  v-for="(item, index) in orderList" :key="index">
					<view class="u-flex u-row-between u-padding-tb-20 u-border-bottom">
						...
						<view class="u-font-28 u-color-major">待核销</view>
					</view>
					<view class="u-padding-tb-10">
						<view class="order-product-item" v-for="(product, i) in item.orderGoods" :key="i" 
							@click="$toPage('/bundle/pages/writeoff_detail/writeoff_detail?id='+item.pick_up_code)">
					...
					<view class="order-button-group">
						<view class="button-item button-item--main" >立即核销</view>
					</view>
				</view>
			</view>
		</mescroll-body>
	...
	export default {
		...
		methods: {
			...
			upCallback(page) {
				...
				this.$u.api.apiStoreWriteOffOrder({
					type: this.tabIndex,
					page: pageNo
				}).then(result => {
