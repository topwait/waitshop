	...
	<view class="container">
		...
		<mescroll-body :top="80" ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" :up="upOption">
			<view class="index-coupon-widget">
				<view class="coupon-item" :class="tabIndex>0?'failure':''" v-for="(item, index) in couponList" :key="index">
					<view class="mod-header-region u-flex">
						<view class="coupon-item__left">
							<block v-if="item.type === 10">
								<wait-price amount="50" :fontColor="tabIndex>0?'#FFF':'#fe574e'" fontWeight="bold" mainSize="58rpx" minorSize="30rpx"></wait-price>
							</block>
							<block v-if="item.type === 20">
								<view class="u-font-weight" :style="{color:tabIndex>0?'#FFF':'#fe574e'}">
									<text style="font-size:58rpx;">{{item.discount}}</text>
									<text class="u-margin-left-4">折</text>
								</view>
							</block>
							<view class="u-font-24 u-margin-top-16" :style="{color:tabIndex>0?'#FFF':'#c36968'}">{{item.use_condition}}</view>
						</view>
						<view class="coupon-item__right">
							<view class="u-font-30 u-line-2" :style="{color:tabIndex>0?'#FFF':'#514545'}">{{item.name}}</view>
							<view class="condition">
								<text class="text u-line-2">{{item.use_goods_type}}</text>
							</view>
							<view class="failure-time">
								失效时间：{{item.expiry_time}}
							</view>
						</view>
					</view>
					<view class="mod-footer-region u-flex u-row-between">
						<view>
							<u-collapse 
								hover-class="none"
								:arrow-color="tabIndex===0?'#333':'#FFF'"
								:head-style="{color:tabIndex===0?'#333':'#FFF', fontSize:'28rpx', justifyContent: 'flex-start'}"
								:body-style="{color:tabIndex===0?'#333':'#FFF', fontSize:'22rpx'}">
						...
						<view class="u-margin-left-30">
							<view class="btn" v-if="tabIndex===0" @click="$toPage('/pages/goods/list/list')">去使用</view>
							<view class="btn" v-if="tabIndex===1">已使用</view>
							<view class="btn" v-if="tabIndex===2">已过期</view>
						</view>
					</view>
				</view>
			</view>
